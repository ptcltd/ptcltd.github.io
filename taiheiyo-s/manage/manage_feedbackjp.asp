...
conn.execute("delete from feedbackjp where id="&request("id")&"")
response.write "<script>alert('delete Succeed!');window.location.href='manage_feedbackjp.asp';</script>"
response.end
end if
%>
...
function shanchu(unid)     
   {     
      if(confirm("\n delete!"))     
      {     
        window.location.href="manage_feedbackjp.asp?flag=del&id="+unid;
      }        
   } 
</script>
<html>
...
%>
  <tr>
    ...
    <td align="center" valign="middle" bgcolor="#FFFFFF"><label>
      <input name="Submit" type="button" class="go-wenbenkuang" value="Detail" onClick="window.location='manage_fbjp.asp?id=<%=rs("id")%>';">

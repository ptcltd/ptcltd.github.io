...
conn.execute("delete from feedbackko where id="&request("id")&"")
response.write "<script>alert('delete Succeed!');window.location.href='manage_feedbackko.asp';</script>"
response.end
end if
%>
...
function shanchu(unid)     
   {     
      if(confirm("\n delete!"))     
      {     
        window.location.href="manage_feedbackko.asp?flag=del&id="+unid;
      }        
   } 
</script>
<html>
...
%>
  <tr>
    ...
    <td align="center" valign="middle" bgcolor="#FFFFFF"><label>
      <input name="Submit" type="button" class="go-wenbenkuang" value="Detail" onClick="window.location='manage_fbko.asp?id=<%=rs("id")%>';">

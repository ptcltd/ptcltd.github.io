...
rs.close
set rs=nothing
response.Write("<script>alert('Modification success!');window.location='manage_producteditjp.asp?id="&id&"';</script>")
end if
...
subcat = new Array();
        ...
        do while not rs.eof 
        %>
subcat[<%=count%>] = new Array("<%= trim(rs("jp_ClassName"))%>","<%= trim(rs("class_id"))%>","<%= trim(rs("id"))%>");
        <%
        count = count + 1
        rs.movenext
        loop
...
function CheckForm()
{
  if (document.myform.jp_title.value=="")
  {
    alert("Please input product name!");
	document.myform.jp_title.focus();
	return false;
  }
...
<table width="670" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
  <form name="myform" method="post" action="manage_producteditjp.asp?action=edit">
  <tr>
  ...
  <tr>
    ...
    <td width="537" height="25" align="left" valign="middle" bgcolor="#f1f1f1"><%
		...
		%>
		    ...
		    do while not rs.eof
			%>
                      <option value="<%=trim(rs("id"))%>" <%if rs("id")=cint(bigid) then%> selected<%end if%>><%=trim(rs("jp_ClassName"))%></option>
                      <%
        ...
        rs.close
			...
			%>
                      <option value="<%=rs("id")%>" <%if rs("id")=cint(smallid) then%> selected<%end if%>><%=rs("jp_ClassName")%></option>
                      <% rs.movenext
				do while not rs.eof%>
                      <option value="<%=rs("id")%>" <%if rs("id")=smallid then%> selected<%end if%>><%=rs("jp_ClassName")%></option>
                      <%
			    	rs.movenext
				loop
			end if
  ...
  <tr>
    ...
    <td height="25" align="left" valign="middle" bgcolor="#f1f1f1"><input name="jp_title" type="text" id="jp_title" value="<%=prs("jp_title")%>" size="30"></td>
  </tr>
  ...
  <tr>
    ...
    <td height="25" align="left" valign="middle" bgcolor="#f1f1f1"><input type="radio" name="flag2" value="1" <%if prs("flag2")="1" then%>checked<%end if%>>
      yes
      <input name="flag2" type="radio" value="0" <%if prs("flag2")<>"1" then%>checked<%end if%>>
      no</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td height="200" align="left" valign="middle" bgcolor="#f1f1f1"><script language="JavaScript">
		   document.write ('<IFRAME ID="content1" src="editor/ewebeditor.asp?id=jp_content&style=edragon" frameborder="0" scrolling="no" width="100%" height="200"></IFRAME>')       
                        </script><input name="Content1" type="hidden" id="Content1" value="">
						<input name="jp_Content" type="hidden" id="jp_Content" value="<%=Server.HTMLEncode(prs("jp_content"))%>"></td>
  </tr>

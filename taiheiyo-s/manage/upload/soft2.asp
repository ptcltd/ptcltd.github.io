...
For Each File in Upload.Files
 ...
 formpath="/pic/movie/" 
 filename=formpath&"big"&session("userid")&year(now)&month(now)&day(now)&hour(now)&minute(now)&second(now)&ranNum&file.Ext
  
 File.SaveAsVirtual filename    ''保存文件
 	response.write "<script>parent.myform.bigimg.value='"&filename&"'</script>"
 set file=nothing
next

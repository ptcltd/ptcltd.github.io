...
For Each File in Upload.Files
 ...
 formpath="/pic/logo/" 
 filename=formpath&session("userid")&file.Ext
  
 File.SaveAsVirtual filename    ''保存文件
 	response.write "<script>parent.myform.logo.value='"&filename&"'</script>"
 set file=nothing
next
set upload=nothing  ''删除此对象
response.write "图片已经成功上传！"
end if%>     <form name="form" method="post" action="?chuan=yes" enctype="multipart/form-data" >
      <input type="hidden" name="act" value="upload">
  <input name="st" type="file" class="wenbenkuang" id="st" size="40">

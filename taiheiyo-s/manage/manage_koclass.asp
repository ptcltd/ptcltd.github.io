...
response.write "<script>alert('delete Succeed!');window.location.href='manage_koclass.asp';</script>"
response.end
end if
id=request("id")
if id<>"" then
...
if ko_classname="" then
response.Write("<script>alert('please input Korean name!');window.history.go(-1);</script>")
response.End()
end if
...
rs.close
set rs=nothing
response.Write("<script>alert('Modification success!');window.location='manage_koclass.asp';</script>")
end if
%>
...
function shanchu(unid)     
   {     
      ...
      {     
        window.location.href="manage_koclass.asp?flag=del&id="+unid;
      }        
   } 
</script></head>
<body>
...
<table border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
  ...
  <tr bgcolor="#FFFFFF"> 
    <td height="25"><div align="center"><strong>English Name</strong></div></td>
    <td height="25"><div align="center"><strong>Korea Name</strong></div></td>
  ...
  %>
  <tr bgcolor="#FFFFFF"> 
  <form action="manage_koclass.asp" method="post">
    <td height="25" align="center"><font color="#ff0000"><%=rs("en_classname")%></font></td>
    <td height="25" align="center"><input style=" color:#FF0000" name="ko_classname" type="text" id="ko_classname" size="23" value="<%=rs("ko_classname")%>"></td>
    <td height="25" align="center"><input name="id" type="hidden" value="<%=rs("id")%>">
      <input name="Submit4" type="button" disabled onClick="window.location='manage_addclassjp.asp?bigclass=<%=rs("id")%>';" value="Add SmallClass">
  ...
  <%
		  ...
		  if srs.bof and srs.eof then
			...
			%>
			<tr bgcolor="#FFFFFF"><form action="manage_koclass.asp" method="post"><td colspan="3" align="right" bgcolor="#F0F0F0"><input name="ko" type="text" disabled class="txtbg" id="ko_classname" style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:11px;" value="<%=srs("en_classname")%>" size="35">
			  <input name="ko_classname" type="text" id="ko_classname" value="<%=srs("ko_classname")%>" size="20">
